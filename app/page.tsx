  ...
  return (
    <>
      ...
      <footer>
        ...
        <div className="footer-links"><Link href="/explore">{language === "en" ? "Explore Bangladesh" : "বাংলাদেশ ঘুরে দেখো"}</Link><Link href="/alphabet">{language === "en" ? "The Bangla alphabet" : "বর্ণমালা"}</Link><Link href="/phrasebook">{language === "en" ? "First phrases" : "বাক্যের ঝুলি"}</Link><Link href="/numbers">{language === "en" ? "Numbers & counting" : "সংখ্যা ও গোনা"}</Link><Link href="/calendar">{language === "en" ? "Days & seasons" : "দিন ও ঋতু"}</Link><Link href="/grammar">{language === "en" ? "How Bangla works" : "ব্যাকরণ"}</Link><a href="https://whc.unesco.org/en/statesparties/bd" target="_blank" rel="noreferrer">UNESCO places</a><a href="https://www.unesco.org/en/days/mother-language" target="_blank" rel="noreferrer">Mother Language Day</a><button type="button" onClick={() => setModal({ type: "grownups" })}>{t.privacy}</button></div>
      </footer>
